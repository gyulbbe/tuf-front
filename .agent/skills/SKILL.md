...
- 사용자, 선수, 후보, 팀장/부팀장처럼 검색해서 선택 후 목록에 추가하는 UI는 추가 버튼 클릭뿐 아니라 Enter 키로도 같은 추가 동작이 되게 구현한다.
- 검색 결과 드롭다운이 열려 있고 결과가 있으면 Enter는 첫 번째 결과를 선택하고 즉시 추가한다.
- 이미 선택된 항목이 있으면 Enter는 선택된 항목을 즉시 추가한다.
- 검색 입력에서 Enter를 처리할 때는 상위 `<form>` 제출로 이어지지 않도록 `preventDefault()`를 적용한다.

## 참가자/사용자 표시명 정책

- 특별한 요청이 없으면 화면에 사용자 실명이나 이름(`name`)을 표시하지 않는다.
- 내부 회원은 화면 표시명으로 로그인 아이디/사용자 아이디(`userId`, `userLoginId`)만 사용한다.
- 토너먼트 대진표, 경기 카드, 점수 제출자, 승인/반려 내역, 결과 슬롯, 참가자 풀에서도 같은 원칙을 적용한다.
- 내부 회원 표시를 위해 `participantName`, `name`, DB PK 값을 직접 노출하지 않는다.
- 외부 참가자처럼 `userId`가 없는 경우에만 `participantName`을 표시한다.
- 백엔드 응답에 `displayName`이 있더라도, 내부 회원의 `displayName`은 아이디 기반 값이어야 한다는 전제를 유지한다. 응답이 실명처럼 보이면 프론트에서 우회하지 말고 백엔드 수정 요청을 먼저 제안한다.
