      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-foreground">
            #{pick.pickNo} · {pick.candidateName}
          </p>
      ...
      try {
        ...
        if (isMissingSessionError(error)) {
          ...
          if (!cancelled) {
            setNotice({
              tone: "neutral",
              text: "보고 있던 드래프트가 삭제되어 이력 목록에서 제거했다.",
            });
          }
          return;
        }
    ...
    <div className="space-y-4">
      <SurfaceCard className="p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-7 text-muted">
              잘못 들어간 픽 기록을 여기서 정리하고, 삭제 직후 같은 드래프트 상세를 다시 읽어서
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          <div className="rounded-[24px] border border-line bg-surface-strong px-4 py-4">
            <p className="text-sm font-semibold text-foreground">선택 드래프트</p>
          ...
          <div className="rounded-[24px] border border-line bg-surface-strong px-4 py-4">
            ...
            <p className="mt-2 text-sm text-muted">현재 드래프트에 쌓인 전체 픽 기록</p>
          </div>
          <div className="rounded-[24px] border border-line bg-surface-strong px-4 py-4">
      ...
      <SurfaceCard className="p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-foreground">드래프트별 픽 기록</p>
            <p className="mt-2 text-sm leading-7 text-muted">
              드래프트를 고르면 아래에 픽 기록이 시간순으로 나온다. 잘못된 기록만 골라서
              삭제하면 된다.
            </p>
          </div>
