...
import {
  buildEntrySubmissionWebSocketUrl,
  type EntrySubmissionEvent,
} from "@/lib/api/entry-submission";

export type EntrySubmissionConnectionState =
  | "connecting"
  | "connected"
  | "reconnecting"
  | "disconnected"
...
type SubscribeOptions = {
  sessionId: number;
  onEvent: (event: EntrySubmissionEvent) => void;
  onStateChange?: (state: EntrySubmissionConnectionState) => void;
...
export type EntrySubmissionSubscription = {
  unsubscribe: () => void;
};
...
function parseFrame(rawFrame: string) {
  ...
  return { command: command.trim(), headers, body } satisfies StompFrame;
}
...
function extractFrames(chunk: string, pending: string) {
  ...
  return { frames, pending: nextPending };
}

export function subscribeToEntrySubmissionSession({
  sessionId,
  onEvent,
  onStateChange,
  onError,
}: SubscribeOptions) {
  const wsUrl = buildEntrySubmissionWebSocketUrl();
  const subscriptionId = `entry-submission-session-${sessionId}`;
  ...
  function handleFrame(frame: StompFrame) {
    ...
    if (frame.command === "CONNECTED") {
      ...
      socket.send(
        buildFrame("SUBSCRIBE", {
          id: subscriptionId,
          destination: `/topic/entry-submissions/${sessionId}`,
        }),
      );
      return;
    }
    ...
    if (frame.command === "MESSAGE") {
      try {
        onEvent(JSON.parse(frame.body) as EntrySubmissionEvent);
      } catch {
        onError?.("엔트리 제출 이벤트를 해석하지 못했습니다.");
      }
      return;
    }
    ...
    if (frame.command === "ERROR") {
      onStateChange?.("error");
      onError?.(frame.body || "엔트리 제출 소켓 연결 중 오류가 발생했습니다.");
    }
  }
  ...
  return { unsubscribe } satisfies EntrySubmissionSubscription;
}
